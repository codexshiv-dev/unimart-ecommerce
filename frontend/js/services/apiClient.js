...
const ApiClient = (() => {
  const request = async (url, options = {}) => {
    ...
    if (!response.ok) {
      // A 401 here only means "session expired" if we previously believed we
      // were logged in (AuthState.isLoggedIn() true). A logged-out visitor's
      // normal GET /api/auth/me 401 on initial page load never reaches this
      // branch, because currentUser is still null at that point - so this
      // can't misfire on the expected, silent, logged-out case.
      if (response.status === 401 && window.AuthState?.isLoggedIn()) {
        window.AuthState.handleSessionExpired();
      }
